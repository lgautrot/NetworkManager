 ...
 * @device: a #NMDeviceVxlan
 *
 * Returns: the source IP address to use in outgoing packet
 *
 * Since: 1.2
 **/
const char *
...
connection_compatible (NMDevice *device, NMConnection *connection, GError **error)
{
	...
	if (setting_hwaddr) {
		if (!nm_utils_hwaddr_matches (setting_hwaddr, -1,
		                              NM_DEVICE_VXLAN_GET_PRIVATE (device)->hw_address, -1)) {
			g_set_error_literal (error, NM_DEVICE_ERROR, NM_DEVICE_ERROR_INCOMPATIBLE_CONNECTION,
...
nm_device_vxlan_class_init (NMDeviceVxlanClass *vxlan_class)
{
	 ...
	 * NMDeviceVxlan:group:
	 *
	 * The unicast destination IP address used in outgoing
	 * packets when the destination link layer address is not known in the VXLAN
	 * device forwarding database or the multicast IP address joined.
	 */
	g_object_class_install_property
	 ...
	 *
	 * The UDP destination port used to communicate with the
	 ...
	 *
	 * The minimum UDP source port used to communicate with the
	 * remote VXLAN tunnel endpoint.
	 *
	 * Since: 1.2
	 */
	 ...
	 *
	 * The maximum UDP source port used to communicate with the
	 * remote VXLAN tunnel endpoint.
	 *
	 * Since: 1.2
	 */
