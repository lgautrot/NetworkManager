...
struct {
	...
	int family;
		...
		{ NM_LINK_TYPE_SIT,    NM_IP_TUNNEL_MODE_SIT,      AF_INET },
};
...
static gboolean
address_equal_pp (NMIPTunnelMode mode, const char *a, const char *b)
{
...
static gboolean
address_equal_pn (NMIPTunnelMode mode, const char *a, const void *b)
{
	char buffer1[sizeof (struct in6_addr)] = { };
	int family;
...
update_properties (NMDevice *device)
{
	...
	/* At the moment we support only tunnels over IPv4 */
	if (!address_equal_pn (priv->mode, priv->local, &local4)) {
	...
	if (!address_equal_pn (priv->mode, priv->remote, &remote4)) {
               ...
               gboolean *out_ignore)
{
	...
	if (connection) {
		...
		g_return_val_if_fail (val >= 0, NULL);
		mode = val;
	}
