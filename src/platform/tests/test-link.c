...
test_bogus(void)
{
	...
	g_assert (!nm_platform_vlan_get_info (NM_PLATFORM_GET, BOGUS_IFINDEX, NULL, NULL, NULL));
...
test_software (NMLinkType link_type, const char *link_typename)
{
	...
	if (link_type == NM_LINK_TYPE_VLAN) {
		g_assert (nm_platform_vlan_get_info (NM_PLATFORM_GET, ifindex, &vlan_parent, &vlan_id, &vlan_flags));
		g_assert_cmpint (vlan_parent, ==, nm_platform_link_get_ifindex (NM_PLATFORM_GET, PARENT_NAME));
		g_assert_cmpint (vlan_id, ==, VLAN_ID);
		g_assert_cmpint (vlan_flags, ==, VLAN_FLAGS);
	}
